...
// credits to
// https://github.com/ikoz/AndroidSubstrate_hookingC_examples/blob/master/nativeHook3/jni/nativeHook3.cy.cpp
...
bool Util::protect(uint32_t *target, int protection) {
  ...
  auto *page_aligned_target = reinterpret_cast<uint32_t *>(
      reinterpret_cast<uint64_t>(target) & ~(kPageSize - 1));
  LOG_DEBUG("Marking target: {} as writable, page aligned: {}",
            fmt::ptr(target), fmt::ptr(page_aligned_target));
  if (::mprotect(page_aligned_target, kPageSize, protection) != 0) {
    // Log error on mprotect!
    LOG_ERROR("Failed to mark: {} (page aligned: {}). err: {}",
              fmt::ptr(target), fmt::ptr(page_aligned_target),
              std::strerror(errno));
    return false;
  }
